...
const standardSidebar = `
        ...
        <div class="nav-item-group" id="asset-nav-group">
  ...
  if (regex.test(content)) {
    ...
    for (let i = 0; i < lines.length; i++) {
      if (lines[i].includes(`href="${targetFile}"`) || lines[i].includes(`href="${file}"`)) {
        ...
        if (assetPages.includes(file)) assetGroupActive = true;
    ...
    if (assetGroupActive) {
      newMenuContent = newMenuContent.replace(
        'class="nav-item-group" id="asset-nav-group"',
        'class="nav-item-group expanded" id="asset-nav-group"'
      );
    }
